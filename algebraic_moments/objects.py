...
class ConcentrationInequality(object):
    ...
    def print_python(self, as_function=None):
        """
        Print Python code for evaluating the concentration inequality.

        args:
            as_function (string, optional): If none, print a script; otherwise, print
                                            a function with this name
        """
        bound_expr, condition_expr = self.build_expressions()

        # Print the function boilerplate if needed
        if as_function is not None and isinstance(as_function, str):
            print("def " + as_function + "(input_moments, input_deterministic):")
            prefix = "    "
        else:
            prefix = ""
        ...
        # Print the docstring
        docstring = (f'{prefix}"""'
                     f"\n{prefix}Evaluate concentration inequality conditions, as auto-generated by"
                     f"\n{prefix}the algebraic_moments package."
                     f"\n\n{prefix}args:"
                     f"\n{prefix}    input_moments: a dictionary indexed by variable name containing the"
                     f"\n{prefix}                   moments needed to compute this inequality"
                     f"\n{prefix}    input_deterministic: a dictionary indexed by variable name"
                     " containing"
                     f"\n{prefix}                   the deterministic values needed to compute this"
                     " inequality"
                     f'\n{prefix}"""')
        print(docstring)

        self._moment_expressions.print_python(prefix=prefix)
        print(f"\n{prefix}# Establish the probability bound.")
        print(f"{prefix}# We need necessary_condition<=0 for this bound to hold.")
        print(f"{prefix}variance = second_moment - first_moment**2")
        print(f"{prefix}probability_bound = " + str(bound_expr))
        print(f"{prefix}necessary_condition = " + str(condition_expr))
...
class MomentExpressions(object):
    ...
    def print_python(self, multi_idx_keys=False, prefix=""):
        """Print python code.
        ...
        print(f"{prefix}# Parse required inputs.")
        for moment in self._moments:
            ...
            print(prefix + str(moment) + " = input_moments[" + dict_input + "]")

        for det_var in self._deterministic_variables:
            print(prefix + str(det_var) +" = input_deterministic[\"" + str(det_var) + "\"]" )

        # Generate constraint expressions.
        print(f"\n{prefix}# Moment expressions.")
        for name, cons in self._moment_expressions.items():
            print(prefix + str(name) + " = " + pycode(cons))
